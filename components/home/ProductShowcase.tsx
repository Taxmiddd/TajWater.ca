...
const iconMap: Record<string, LucideIcon> = {
  ...
  accessories: Building2,
}

const themeMap: Record<string, { badge: string; badgeColor: string; gradient: string; iconColor: string }> = {
  water: {
    badge: 'Best Seller',
    badgeColor: '#0097a7',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {productList.map((product, i) => {
            const theme = themeMap[product.category] || themeMap.water
            const Icon = iconMap[product.category] || Droplets
            const features = product.description.split('.').filter((s: string) => s.trim().length > 0).slice(0, 3)
            ...
            return (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <p className="text-[#4a7fa5] text-sm leading-relaxed mb-4 line-clamp-2">{product.description}</p>

                  <ul className="space-y-2 mb-5">
                    {features.map((f: string) => (
                      <li key={f} className="flex items-center gap-2 text-sm text-[#0c2340]">
                        <span className="w-4 h-4 rounded-full flex items-center justify-center text-[9px] text-white flex-shrink-0" style={{ background: theme.iconColor }}>✓</span>
